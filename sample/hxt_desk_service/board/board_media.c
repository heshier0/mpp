...
static BOOL start_venc()
{
    ...
    ret_val = SAMPLE_COMM_VENC_SnapStart(venc_chn[2], &picture_size, 0);
    if (HI_SUCCESS != ret_val)
    {
    ...
    ret_val = SAMPLE_COMM_VENC_SnapStart(venc_chn[3], &picture_size, 0);
    if (HI_SUCCESS != ret_val)
    {
        SAMPLE_PRT("start sanp chn %d failed, s32Ret: 0x%x !\n", venc_chn[3], ret_val);
        SAMPLE_COMM_VENC_Stop(venc_chn[0]);
        SAMPLE_COMM_VENC_Stop(venc_chn[1]);
        SAMPLE_COMM_VENC_Stop(venc_chn[2]);
        return FALSE;
    }
...
static void board_get_venc_stream()
{
    VENC_CHN venc_chn = -1;
...
static void board_store_yuv_from_vpss_chn()
{
    HI_S32 ret_val;
    VPSS_GRP vpss_grp[2] = {0, 1};
    HI_S32 time_out = 20000;
    VIDEO_FRAME_INFO_S  color_frame;
	VIDEO_FRAME_INFO_S  gray_frame;
    ...
    while(g_video_status)
    {
        ret_val = HI_MPI_VPSS_GetChnFrame(vpss_grp[0], 2, &color_frame, time_out);
		if (HI_SUCCESS != ret_val)
		{
        ...
        ret = select(fd + 1, NULL, &write_fds, NULL, &timeout_val);
        if (ret < 0)
        {
