...
#include "layer12.h"
	...
	static const unsigned short frameSizeTab[9][15][3] =
	{
		/* MPEG1 */
...
static int  pvmp3_CalcFrameBytes(int version_x, int layer_description, int bitrate_index, int sampling_frequency,
                            int padding)
{
    int frmBytes = (int)frameSizeTab[(version_x) * 3 + layer_description
                                           - 1][bitrate_index][sampling_frequency] + (padding ? 1 : 0);

    if ((1 == layer_description) && (1 == padding))
    {
...
int pvmp3_UnpackFrameHeader(tPVMP3DecoderExternal *pExt,  void				  *pMem)
{
    ...
    frmBytes = pvmp3_CalcFrameBytes(info->version_x, info->layer_description, info->bitrate_index,
                                      info->sampling_frequency, info->padding);
