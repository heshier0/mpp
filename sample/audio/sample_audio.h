...
HI_S32 SAMPLE_AUDIO_AdecAo(HI_VOID)
{
    ...
    stAioAttr.enSamplerate   = AUDIO_SAMPLE_RATE_16000;//AUDIO_SAMPLE_RATE_48000;
    stAioAttr.enBitwidth     = AUDIO_BIT_WIDTH_16;
    stAioAttr.enWorkmode     = AIO_MODE_I2S_MASTER;
    stAioAttr.enSoundmode    = AUDIO_SOUND_MODE_STEREO;
    stAioAttr.u32EXFlag      = 0;
    ...
    HI_S32 volDB = 0;
    s32Ret = HI_MPI_AO_GetVolume( AoDev, &volDB);
    printf("voluem db is %d\n", volDB);
...
/******************************************************************************
* function : Open Prompt Tone File
******************************************************************************/
static FILE* SAMPLE_AUDIO_OpenPromptToneFile(HI_CHAR* aszFileName)
{
    FILE* pfd;
...
* function : file -> Adec -> Ao
******************************************************************************/
HI_S32 PLAY_PROMPT_TONE(HI_CHAR* aszFileName)
{
    HI_S32      s32Ret;
    AO_CHN      AoChn = 0;
    ADEC_CHN    AdChn = 0;
    ...
    pfd = SAMPLE_AUDIO_OpenPromptToneFile(aszFileName);
    if (!pfd)
    {
        SAMPLE_DBG(HI_FAILURE);
        goto ADECAO_ERR0;
...
HI_S32 SAMPLE_AUDIO_AiAenc(HI_VOID)
{
    ...
    ********************************************/
    ...
    s32Ret = SAMPLE_COMM_AUDIO_StartAenc(s32AencChnCnt, &stAioAttr, PT_ADPCMA);
    if (s32Ret != HI_SUCCESS)
    {
        SAMPLE_DBG(s32Ret);
        goto AIAENC_ERR5;
    ...
    ********************************************/
    ...
    {
        s32Ret = SAMPLE_COMM_AUDIO_StartAdec(AdChn, PT_ADPCMA);
        if (s32Ret != HI_SUCCESS)
        {
            SAMPLE_DBG(s32Ret);
            goto AIAENC_ERR3;
        ...
        pfd = SAMPLE_AUDIO_OpenAencFile(AdChn, PT_ADPCMA);
        if (!pfd)
        {
            SAMPLE_DBG(HI_FAILURE);
            goto AIAENC_ERR1;
...
HI_S32 SAMPLE_AUDIO_AiAo(HI_VOID)
{
    ...
    s32Ret = HI_MPI_AO_SetVolume(AoDev, 5);
    if (s32Ret != HI_SUCCESS)
    {
...
HI_VOID SAMPLE_AUDIO_Usage(HI_VOID)
{
    ...
    printf("\t7:  decode audio, Usage:./sample_audio <index> <file>\n");
}
...
//    if (2 != argc)
    ...
    if (u32Index == 7)
    {
        ...
        snprintf(aszFileName, FILE_NAME_LEN, "%s", argv[2]);
    }
